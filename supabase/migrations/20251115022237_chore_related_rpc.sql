...
BEGIN
  ...
  -- Optional: ensure user belongs to this home, or is creator, etc.
  -- PERFORM public._assert_memberships(v_user_id, v_existing.home_id);
  ...
  ELSE
    v_photo_delta := 0;   -- either no photo both times, or replace photo → same slot
  END IF;
...
BEGIN
  ...
  IF v_chore.recurrence = 'none' THEN
    -- Idempotent: if already completed, do nothing and tell client
    IF v_chore.state = 'completed' THEN
      RETURN jsonb_build_object(
    ...
    -- Decrement active chore count for this home
    PERFORM public._home_usage_increment(v_chore.home_id, -1, 0);

    -- chores_events_trigger will log a 'complete' event from state change
    RETURN jsonb_build_object(
...
BEGIN
  ...
  ELSIF TG_OP = 'UPDATE' THEN
    ...
    ----------------------------------------------------------------
    -- Priority:
    -- 1) Recurring completion inferred from next_occurrence moving forward
    -- 2) Assignee change (assign / unassign)
    -- 3) Recurrence config change
    -- 4) Generic state change update
    ----------------------------------------------------------------

    -- 1️⃣ Completion inferred from next_occurrence advancing for recurring chores
    ...
    -- 3️⃣ Recurrence config changed (e.g. weekly → every_2_weeks)
    ...
    -- 4️⃣ Fallback: some other meaningful state change
